...
def clean_emojis_and_punctuations(text):
...
def clean_text(text, remove_special=False, clean_emojis=False, lowercase=False, correct_spelling= False, lemmatize=False, remove_stops=False):
    if not isinstance(text, str):
        return str(text)
    if remove_special:
        text = clean_text_regex(text)
    if clean_emojis:
        text = clean_emojis_and_punctuations(text)
...
DEFAULT_OUTPUT_FILE = r"./data/cleaned_youtube_comments.csv"
